...
def process_stocks_from_txt(client, input_file, output_file, date_str, n_days):
    """
    从文本文件读取股票代码列表，计算每只股票的表现，并输出到Excel文件

    参数:
    client: 初始化后的客户端对象
    input_file: 输入文本文件路径（每行一个股票代码）
...
if __name__ == "__main__":
    ...
    # 输入参数
    input_txt = "stock_codes.txt"  # 每行一个股票代码的文本文件
    output_excel = "股票表现分析结果.xlsx"
    start_date = "2025-08-26"  # 起始日期
    trading_days = 2  # 分析后5个交易日的表现

    # 处理股票并输出结果
    process_stocks_from_txt(client, input_txt, output_excel, start_date, trading_days)