...
def save_stock_features(code, offset=400):
    """
    单支股票任务（多线程执行）
    """
    try:
        client = Quotes.factory(market="std")   # 每个线程独立一个 client，避免冲突
...
def run_all_codes(offset=400, max_workers=20):
    codes = get_all_codes()
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        print("*"*100)
        future_to_code = {executor.submit(save_stock_features, code, offset): code for code in codes}

        for future in as_completed(future_to_code):
            code = future_to_code[future]
            res_code, msg = future.result()

            if "OK" in msg:
                print(f"✔ {res_code} 完成")
                success.append(res_code)
            else:
                print(f"❌ {res_code} 失败：{msg}")
                fail.append(res_code)
